...
export default function AuthControls({ onAuthChanged }: Props) {
  ...
  useEffect(() => {
    ...
    fetchSession()
      .then((session) => {
        setLoggedIn(true);
        setWhoami(session?.username || session?.subject || 'operator');
      })
      .catch(() => {
        clearAuth();
        setLoggedIn(false);
  ...
  const onLogin = async (event: FormEvent) => {
    ...
    try {
      ...
      setLoggedIn(true);
      setWhoami(session?.username || session?.subject || 'operator');
      setMessage(`Logged in via ${mode}.`);
      onAuthChanged?.();
    } catch (e: any) {
      clearAuth();
      setLoggedIn(false);
      setWhoami('');
      setMessage(`Login failed: ${e?.message || 'unauthorized'}`);
      onAuthChanged?.();
    }
  };
    ...
    <form className="auth-inline" onSubmit={onLogin}>
      {!loggedIn && (
        <>
          ...
          <button className="btn" type="submit" disabled={checking}>Login</button>
        </>
      )}

      {loggedIn && (
        <>
          <span className="hint auth-msg">Signed in as {whoami}</span>
          <button className="btn" type="button" onClick={onLogout}>Logout</button>
        </>
      )}

      {!loggedIn && checking && <span className="hint auth-msg">Checking session…</span>}
